#ifndef NORMALTYPE_H
#define NORMALTYPE_H
#include <iostream>
#include "Pokemon.h"


#pragma once

class NormalType: public Pokemon
{
public:
    NormalType(string n, string nickn, int a, int spA, int d, int spD, int spd, int HP): Pokemon(n, nickn, a, spA, d, spD, spd, HP){
        weakness= "Fighting";
        resistance= "None";
        immunity= "Ghost";
        strength= "None";
        statTotal= a + spA +d + spD + spd + HP;
    };
    ~NormalType(){};
    string getStats()const;
    string evaluateStats()const;

private:
string weakness;
string resistance;
string immunity;
string strength;
int statTotal;
};

string NormalType::getStats()const{
    return Pokemon::getStats() + "Stat Total: " + to_string(statTotal)+ "\n" + "Weak to: " + weakness + "\n" + 
    "Resists: " + resistance +"\n" + "Strong against: " + strength + "\n" + "Immune tp: " + immunity + "\n";
}

string NormalType::evaluateStats()const{
    string totalEval;
   
   if (statTotal>= 450){
    totalEval= "Good stat total overall!";
   }
   else if(statTotal <450 && statTotal >= 350){
    totalEval = "Decent stats overall!";
   }
   else{
    totalEval= "Bad overall stats!";
   }
   
    return Pokemon::evaluateStats() + totalEval + "\n";
}

#endif